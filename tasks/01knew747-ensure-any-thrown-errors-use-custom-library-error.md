...
- [x] Inventory every location in the library that throws an error
- [x] Identify the custom error classes that already exist and the gaps in current coverage
- [x] Add any missing custom error types needed to represent current failure modes clearly
- [x] Replace generic thrown errors with the appropriate declared custom error class
- [x] Ensure each custom error includes a stable name/message and any useful contextual fields supported by the library's design
- [x] Add a one-line instruction to `CLAUDE.md` requiring library-thrown errors to use custom error classes declared in this package
- [x] Add a matching one-line instruction to `AGENTS.md` requiring library-thrown errors to use custom error classes declared in this package
- [x] Update exports so any new custom error types are part of the public API if callers need to narrow on them
- [x] Add or update tests covering each affected error path and asserting the thrown error type
- [x] Run the full project verification to confirm lint, tests, and build still pass
