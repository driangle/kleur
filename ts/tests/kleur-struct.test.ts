...
describe("KleurStruct", () => {
  describe("construction and clamping", () => {
    ...
    it("defaults alpha to 1", () => {
      const c = new KleurStruct(0, 0, 0);
      expect(c.a).toBe(1);
    });

    it("accepts explicit alpha", () => {
      const c = new KleurStruct(0, 0, 0, 0.5);
      expect(c.a).toBe(0.5);
    });

    it("clamps RGB below 0", () => {
      const c = new KleurStruct(-10, -1, -255);
      expect(c.r).toBe(0);
      expect(c.g).toBe(0);
      expect(c.b).toBe(0);
    });
    ...
    it("clamps RGB above 255", () => {
      const c = new KleurStruct(300, 256, 999);
      expect(c.r).toBe(255);
      expect(c.g).toBe(255);
      expect(c.b).toBe(255);
    });
    ...
    it("rounds fractional RGB values", () => {
      const c = new KleurStruct(66.7, 135.2, 244.5);
      expect(c.r).toBe(67);
      expect(c.g).toBe(135);
      expect(c.b).toBe(245);
    });
    ...
    it("clamps alpha above 1", () => {
      const c = new KleurStruct(0, 0, 0, 1.5);
      expect(c.a).toBe(1);
    });
  });
  ...
  describe("channel getters", () => {
    it("returns RGB channels", () => {
      const c = new KleurStruct(10, 20, 30, 0.8);
  ...
  describe("immutable setters", () => {
    const base = new KleurStruct(100, 150, 200, 0.9);
  ...
  describe("edge cases", () => {
    ...
    it("fully transparent", () => {
      const transparent = new KleurStruct(255, 0, 0, 0);
  ...
  describe("HSL round-tripping", () => {
    ...
    for (const { name, r, g, b } of testCases) {
      it(`round-trips ${name} within +/-1`, () => {
        const original = new KleurStruct(r, g, b);
        const h = original.hue();
        const s = original.saturation();
        const l = original.lightness();
        const rgb = hslToRgb(h, s, l);
        const roundTripped = new KleurStruct(rgb.r, rgb.g, rgb.b);
